...
pausing for a human when both retries run out.
...
| File | What it is |
|---|---|
| `graph.py` | The compiled LangGraph agent — `build_graph()` returns `(graph, index, reranker, ingest)` |
| `app.py` | Streamlit chat UI: upload docs, chat, live agent trace, human-in-the-loop approval box |
